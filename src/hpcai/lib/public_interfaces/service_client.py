...
class ModelCreationCancelled(Exception):
    def __init__(self):
        super().__init__("Model creation cancelled by user")

    def __str__(self):
        return "Model creation cancelled by user"
...
class ServiceClient(TelemetryProvider, QueueStateObserver):
    ...
    def __init__(self, **kwargs: Any):
        ...
        # Honor caller-provided strict validation flag while defaulting to True
        strict_validation = kwargs.pop("_strict_response_validation", True)
    ...
    def create_training_client_from_state(self, path: str) -> TrainingClient:
        ...
        training_run = rest_client.get_training_run_by_checkpoint_path(path).result()
    ...
    async def create_training_client_from_state_async(self, path: str) -> TrainingClient:
        rest_client = self.create_rest_client()
        training_run = await rest_client.get_training_run_by_checkpoint_path_async(path)
...
def _get_default_headers() -> dict[str, str]:
    ...
    if (api_key := os.environ.get("HPCAI_API_KEY", "")) and "X-API-Key" not in headers:
