...
from hpcai.cookbook.rl.data_processing import (
    assemble_training_data,
    compute_advantages,
    remove_constant_reward_groups,
)
from hpcai.cookbook.rl.metric_util import RLTestSetEvaluator, compute_trajectory_metrics
from hpcai.cookbook.rl.metrics import (
    compute_kl_sample_train,
    compute_post_kl,
    compute_sampling_client_metrics,
    incorporate_kl_penalty,
)
from hpcai.cookbook.rl.rollouts import do_group_rollout
from hpcai.cookbook.rl.types import (
    EnvGroupBuilder,
    RLDataset,
    RLDatasetBuilder,
    TrajectoryGroup,
...
from hpcai.cookbook.utils import logtree, ml_log
from hpcai.cookbook.utils.misc_utils import safezip, split_list, timed, all_same
from hpcai.cookbook.utils.trace import scope, update_scope_context, trace_init
...
def _remove_mask(datum: hpcai.Datum) -> hpcai.Datum:
    return hpcai.Datum(
...
def _training_logprobs_from_fwd_bwd(
    fwd_bwd_result: hpcai.ForwardBackwardOutput,
) -> list[torch.Tensor]:
...
async def train_step(
    data_D: List[hpcai.Datum],
    training_client: hpcai.TrainingClient,
    learning_rate: float,
    num_substeps: int,
    loss_fn: LossFnType,
) -> List[torch.Tensor]:
    ...
    adam_params = hpcai.AdamParams(learning_rate=learning_rate, beta1=0.9, beta2=0.95, eps=1e-8)
...
async def save_checkpoint_and_get_sampling_client(
    ...
    save_every: int,
    start_batch: int = 0,
) -> tuple[hpcai.SamplingClient, dict[str, Any]]:
    metrics = {}
...
) -> tuple[list[hpcai.Datum], dict[str, Any]]:
...
async def do_train_step_streaming_and_get_sampling_client(
    ...
    for i_substep in range(cfg.num_substeps):
        ...
        wrapped_trajectory_groups = []
        forward_backward_futures: list[hpcai.APIFuture[hpcai.ForwardBackwardOutput]] = []
        i_minibatch = 0
...
async def main(
    ...
    if cfg.enable_trace:
        ...
        logger.info(
            f"Run `python hpcai.cookbook/utils/trace.py {trace_events_path} trace.json` and visualize in chrome://tracing or https://ui.perfetto.dev/"
        )
    ...
    with context_manager.UnloadContextManager(training_client):
        ...
        if maybe_test_dataset is not None:
            evaluators.append(RLTestSetEvaluator(maybe_test_dataset, max_tokens=cfg.max_tokens, max_samples_to_eval=8))  # set to -1 to eval all samples
